...
namespace GroupPlus.Business.Repository.CompanyManagement
{
    ...
    {
        private readonly IPlugRepository<Department> _repository;
        ...
        public Department GetDepartment(int departmentId)
        {
            try
            {
                return GetDepartments().Find(k => k.DepartmentId == departmentId) ?? new Department();
            }
            catch (Exception ex)
            {
        ...
        public List<Department> GetDepartments()
        {
            try
            {
                if (!(CacheManager.GetCache("ccDepartmentList") is List<Department> settings) ||
                    settings.IsNullOrEmpty())
                {
                    var myItemList = _repository.GetAll().OrderBy(m => m.DepartmentId);
                    if (!myItemList.Any()) return new List<Department>();
                    settings = myItemList.ToList();
                    if (settings.IsNullOrEmpty())
                        return new List<Department>();
                    CacheManager.SetCache("ccDepartmentList", settings, DateTime.Now.AddYears(1));
                }
                return settings;
            }
            catch (Exception ex)
        ...
        internal void resetCache()
        {
            try
            {
                HelperMethods.clearCache("ccDepartmentList");
                GetDepartments();
            }
            catch (Exception)
            {
        ...
        public SettingRegRespObj AddDepartment(RegDepartmentObj regObj)
        {
            ...
            try
            {
                ...
                if (IsDepartmentDuplicate(regObj.Name, 1, ref response)) return response;
                ...
                var added = _repository.Add(department);

                _uoWork.SaveChanges();

                if (added.DepartmentId < 1)
                {
                ...
                response.SettingId = added.DepartmentId;
            }
        ...
        public SettingRegRespObj UpdateDepartment(EditDepartmentObj regObj)
        {
            var response = new SettingRegRespObj
            {
                Status = new APIResponseStatus
            ...
            try
            {
                ...
                var thisDepartment = getDepartmentInfo(regObj.DepartmentId);

                if (thisDepartment == null)
                {
                    response.Status.Message.FriendlyMessage =
                        "No Department Information found for the specified Department Id";
                    response.Status.Message.TechnicalMessage = "No Department Information found!";
                    return response;
                }

                if (IsDepartmentDuplicate(regObj.Name, 2, ref response)) return response;
                thisDepartment.Name = regObj.Name;
                thisDepartment.Status = (ItemStatus) regObj.Status;
                var added = _repository.Update(thisDepartment);
                _uoWork.SaveChanges();
        ...
        public SettingRegRespObj DeleteDepartment(DeleteDepartmentObj regObj)
        {
            var response = new SettingRegRespObj
            {
                Status = new APIResponseStatus
            ...
            try
            {
                ...
                thisDepartment.Name = thisDepartment.Name + "_Deleted_" + DateTime.Now.ToString("yyyy_MM_dd_hh_mm_ss");
                thisDepartment.Status = ItemStatus.Deleted;
                var added = _repository.Update(thisDepartment);
                _uoWork.SaveChanges();
        ...
        private bool IsDepartmentDuplicate(string name, int callType, ref SettingRegRespObj response)
        {
            try
            {
                var sql1 =
                    $"Select coalesce(Count(\"DepartmentId\"), 0) FROM  \"GPlus\".\"Department\"  WHERE lower(\"Name\") = lower('{name.Replace("'", "''")}')";
                ...
                if (check[0] > 0)
                    ...
                    {
                        response.Status.Message.FriendlyMessage = "Duplicate Error! Department Name already exist";
                        response.Status.Message.TechnicalMessage = "Duplicate Error! Department Name already exist";
                        return true;
                    }
                return false;
            }
        ...
        private Department getDepartmentInfo(int departmentId)
        {
            try
            {
                var sql1 = $"SELECT *  FROM  \"GPlus\".\"Department\" WHERE  \"DepartmentId\" = {departmentId};";
                var agentInfos = _repository.RepositoryContext().Database.SqlQuery<Department>(sql1).ToList();
