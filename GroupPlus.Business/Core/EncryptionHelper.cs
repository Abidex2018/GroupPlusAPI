...
namespace GroupPlus.Business.Core
{
...

    {
...

        {
            ...
            {
                var newPassword = IdGenerator.RandCodeGenMixed(length);

                if (!string.IsNullOrEmpty(newPassword) && newPassword.Length == length)
                ...
                {
                    var value = numbers[random.Next(0, numbers.Length)];
