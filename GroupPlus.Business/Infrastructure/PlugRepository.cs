...
{
    internal class PlugRepository<T> : IPlugRepository<T> where T : class
    {
        private readonly IPlugUoWork _context;
        ...
        public T Update(T entity)
        {
            ...
            _context.Context.PlugDbContext.Entry(entity).State = EntityState.Modified;
            return updated;
        }
        ...
        public IEnumerable<T> UpdateRange(List<T> entities)
        {
            ...
            foreach (var item in entities)
            {
                ...
                _context.Context.PlugDbContext.Entry(item).State = EntityState.Modified;
                retVals.Add(updated);
            }
            return retVals;
        }
        ...
        public IEnumerable<T> Get<TOrderBy>(Expression<Func<T, TOrderBy>> orderBy, int pageIndex, int pageSize,
            SortOrder sortOrder = SortOrder.Ascending)
        {
            return sortOrder == SortOrder.Ascending
                ? GetAll().OrderBy(orderBy).Skip((pageIndex - 1) * pageSize).Take(pageSize).AsEnumerable()
                : GetAll().OrderByDescending(orderBy).Skip((pageIndex - 1) * pageSize).Take(pageSize).AsEnumerable();
        }

        public IEnumerable<T> Get<TOrderBy>(Expression<Func<T, bool>> criteria, Expression<Func<T, TOrderBy>> orderBy,
            int pageIndex, int pageSize, SortOrder sortOrder = SortOrder.Ascending, string includeProperties = "")
        {
            var filtValue = GetAll(criteria, includeProperties);
            if (filtValue == null) return null;
            return sortOrder == SortOrder.Ascending
                ? filtValue.OrderBy(orderBy).Skip((pageIndex - 1) * pageSize).Take(pageSize).AsEnumerable()
                : filtValue.OrderByDescending(orderBy).Skip((pageIndex - 1) * pageSize).Take(pageSize).AsEnumerable();
        }
