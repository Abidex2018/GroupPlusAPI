...
namespace GroupPlus.Common
{
    ...
    {
        readonly double _compareValue;

        public CheckAmountAttribute(double compareValue)
        {
            _compareValue = compareValue;
        }
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            return (decimal)value >= (decimal)_compareValue ? ValidationResult.Success : new ValidationResult(ErrorMessageString);
        }
    }
    [AttributeUsage(AttributeTargets.Property, AllowMultiple = false)]
